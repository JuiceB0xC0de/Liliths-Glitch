...
PCAP_STORAGE_DIR = os.environ.get("PCAP_STORAGE_DIR", "/tmp/pcap_captures")
...
# ============================================================================
# PCAP STREAMING ENDPOINTS - THE MAGIC HAPPENS HERE 🔥
# ============================================================================

@app.route("/api/pcap/stream", methods=["POST"])
def receive_pcap_stream():
    """
    Receive streaming PCAP data from Mac and save it to Kali.
    This is the endpoint that receives live WiFi capture data!
    """
    try:
        # Generate unique filename with timestamp
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        pcap_filename = f"mac_capture_{timestamp}.pcap"
        pcap_path = os.path.join(PCAP_STORAGE_DIR, pcap_filename)
        
        logger.info(f"🔥 Receiving PCAP stream -> {pcap_path}")
        
        # Get the raw binary data from the request
        pcap_data = request.get_data()
        bytes_received = len(pcap_data)
        
        # Write to file
        with open(pcap_path, 'wb') as f:
            f.write(pcap_data)
        
        logger.info(f"✅ Saved {bytes_received} bytes to {pcap_path}")
        
        return jsonify({
            "success": True,
            "message": "PCAP data received and saved",
        ...
        logger.error(f"Error receiving PCAP stream: {str(e)}")
        logger.error(traceback.format_exc())
        return jsonify({
            "success": False,
...
def analyze_pcap():
    """
    Analyze a PCAP file with tshark or other tools.
    """
    try:
        params = request.json
        ...
        if not pcap_file:
            return jsonify({
                "error": "pcap_file parameter is required"
            }), 400
        
        # Construct tshark command based on analysis type
        if analysis_type == "summary":
            command = f"tshark -r {pcap_file} -q -z io,phs"
        elif analysis_type == "conversations":
            command = f"tshark -r {pcap_file} -q -z conv,ip"
        elif analysis_type == "protocols":
            command = f"tshark -r {pcap_file} -q -z io,stat,0"
        elif analysis_type == "detailed":
            command = f"tshark -r {pcap_file} -V"
        elif analysis_type == "custom":
            if filter_expr:
                command = f"tshark -r {pcap_file} -Y '{filter_expr}'"
            else:
                command = f"tshark -r {pcap_file}"
        else:
            command = f"tshark -r {pcap_file}"
        
        logger.info(f"Analyzing PCAP: {command}")
...
def list_pcap_files():
    """
    List all PCAP files in the storage directory.
    """
    try:
        files = []
        for filename in os.listdir(PCAP_STORAGE_DIR):
            if filename.endswith('.pcap') or filename.endswith('.pcapng'):
                filepath = os.path.join(PCAP_STORAGE_DIR, filename)
...
def health_check():
    ...
    essential_tools = ["nmap", "gobuster", "dirb", "nikto", "tshark"]
    tools_status = {}
...
if __name__ == "__main__":
    ...
    logger.info(f"📁 PCAP storage directory: {PCAP_STORAGE_DIR}")
